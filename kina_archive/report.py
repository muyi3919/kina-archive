...
class ReportGenerator:
    ...
    def generate(self, output_path: str = "archive_report.html"):
        ...
        content_html = ""
        for (url,) in urls:
            content_html += self._render_url_section(conn, url)
...
<div class="stat-card"><div class="number">{changes}</div><div class="label">检测到变更</div></div>
    ...
    def _render_url_section(self, conn, url: str) -> str:
        ...
        for snap in snapshots:
            ...
            timeline += '<div class="snapshot"><img src="{}" loading="lazy"><div class="meta">{}</div><div class="hash">{}...</div></div>'.format(ip, ts, ih)
        timeline += '</div>'
        ...
        if changes:
            for ch in changes:
                ...
                ch_html += '<div class="change-item"><span class="sim {}">{:.1%}</span><img class="diff-thumb" src="{}"><span>{}</span></div>'.format(cls, sim, dp, da)
        else:
